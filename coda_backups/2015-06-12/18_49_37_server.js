...
function makeShort( longUrl, player ){
    ...
    request.execute(function(response) 
    {
        ...
        if(response.id != null){
            if(player == "playerOne"){
	            document.getElementById('qrcode_left') = document.getElementById('qrcode_left') + response.id;
            }else{
	            document.getElementById('qrcode_right') = document.getElementById('qrcode_right') + response.id;
            }
        }else{
...
window.addEventListener("load", function() {
	...
	gapi.client.setApiKey('AIzaSyBlDRF4q7uqZ-EazSB8QuitbYpof4h1B8E');
